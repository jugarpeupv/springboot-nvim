...
local function new_boot_run(args)
  local project_root = get_spring_boot_project_root()
  ...
  if maven_file == "" and gradle_file == "" then
    print("Not in a Spring Boot project")
    return
  end
  ...
  if gradle_file ~= "" then
    ...
    if project_root then
      ...
      vim.cmd(cd_cmd)
      local build_cmd =
      ':call jobsend(b:terminal_job_id, "./gradlew build --continuous --parallel --build-cache --configuration-cache\\n")'
      vim.cmd(build_cmd)
      vim.cmd("wincmd k")
    else
      print("Not in a Spring Boot project")
